...
class RoboFile extends \Robo\Tasks
{
    ...
    ])
    {
        ...
        $this->_exec("mysql -u" . MYSQL_USER . " -p" . MYSQL_PASS . " -h " . MYSQL_HOST . " -e 'create user if not exists " . $opts['wp-db-name'] . "'");
        $this->_exec("mysql -u" . MYSQL_USER . " -p" . MYSQL_PASS . " -h " . MYSQL_HOST . " -e 'create database if not exists " . $opts['wp-db-name'] . "'");
        $this->_exec('mysql -u' . MYSQL_USER . ' -p' . MYSQL_PASS . ' -h ' . MYSQL_HOST . ' -e "grant all privileges on ' . $opts['wp-db-name']
            . ' . * to ' . $opts['wp-db-name'] . "@localhost identified by '" . $opts['wp-db-name'] . "'\"");
        $this->_exec("mysql -u" . MYSQL_USER . " -p" . MYSQL_PASS . " -h " . MYSQL_HOST . " -e 'flush privileges'");

        $this->wp('core download --version=4.9 --locale=en_US --force');
        $this->wp('core config --dbname=' . $opts['wp-db-name'] . ' --dbuser=' . $opts['wp-db-name'] . ' --dbpass=' . $opts['wp-db-name'] . ' --dbhost='.MYSQL_HOST);
        $this->wp('db drop --yes');
